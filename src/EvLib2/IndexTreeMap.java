...
implements NavigableMap<K, V>, Cloneable, java.io.Serializable
{
	 ...
	 *             does not permit null keys
	 */
	final int getIndexOf(Object key){
		// Offload comparator-based version for sake of performance
		if(comparator != null) return getIndexOfUsingComparator(key);
		if(key == null) throw new NullPointerException();
		@SuppressWarnings("unchecked")
		Comparable<? super K> k = (Comparable<? super K>)key;
		Entry<K, V> p = root;
	...
	/**
	 * Version of getIndexOf using comparator. Split off from getIndexOf
	 * for performance. (This is not worth doing for most methods,
	 * that are less dependent on comparator performance, but is
	 * worthwhile here.)
	 */
	final int getIndexOfUsingComparator(Object key){
		@SuppressWarnings("unchecked")
		K k = (K)key;
		Comparator<? super K> cpr = comparator;
		if(cpr != null) {
	...
	implements NavigableMap<K, V>, java.io.Serializable{
		...
		final IndexTreeMap.Entry<K, V> absLowest(){
			IndexTreeMap.Entry<K, V> e = (fromStart ? m.getFirstEntry() : (loInclusive ? m.getCeilingEntry(lo) : m.getHigherEntry(lo)));
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final IndexTreeMap.Entry<K, V> absHighest(){
			IndexTreeMap.Entry<K, V> e = (toEnd ? m.getLastEntry() : (hiInclusive ? m.getFloorEntry(hi) : m.getLowerEntry(hi)));
	 ...
	 */
	@SuppressWarnings("unchecked") private final Entry<K, V> buildFromSorted(int level, int lo, int hi,
	...
	static class IndexTreeMapSpliterator<K, V>{
		...
		IndexTreeMap.Entry<K, V> current; // traverser; initially first node in
											// range
